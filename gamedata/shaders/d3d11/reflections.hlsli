...
float3 BiteralReflectionsFiler(float2 TexCoord, Texture2D image) {
    float4 Reflections = image.SampleLevel(smp_rtlinear, TexCoord, 0.0f);
	...
	for(int dx = -1; dx <= 1; ++dx) {
		for(int dy = -1; dy <= 1; ++dy) {
			...
			float4 Sample = image.SampleLevel(smp_rtlinear, Offset, 0.0f);
