...
void CShootingObject::LoadFireParams	(LPCSTR section, LPCSTR prefix)
{
	...
	s_sHitPower			= pSettings->r_string_wb(section, xr_strconcat(full_name, prefix, "hit_power"));//читаем строку силы хита пули оружия
	...
	m_bUseAimBullet		= pSettings->r_bool		(section, xr_strconcat(full_name, prefix, "use_aim_bullet"));
	if (m_bUseAimBullet)
	{
		m_fTimeToAim		= pSettings->r_float	(section, xr_strconcat(full_name, prefix, "time_to_aim"));
	}
}
