...
void CObjectHandlerPlanner::add_evaluators		(CMissile *missile)
{
	...
	add_evaluator		(uid(id,eWorldPropertyHidden)		,new CObjectPropertyEvaluatorMissile(missile,m_object,MS_HIDDEN));
	add_evaluator		(uid(id,eWorldPropertyThrowStarted)	,new CObjectPropertyEvaluatorMissile(missile,m_object,MS_THREATEN));
	add_evaluator		(uid(id,eWorldPropertyThrowIdle)	,new CObjectPropertyEvaluatorMissile(missile,m_object,MS_THROW));
	add_evaluator		(uid(id,eWorldPropertyThrow)		,new CObjectPropertyEvaluatorMissile(missile,m_object,MS_END));
...
void CObjectHandlerPlanner::add_operators		(CMissile *missile)
{
	...
	// idle
	action				= new CSObjectActionBase(missile,m_object,&m_storage,"idle");
	add_condition		(action,id,eWorldPropertyHidden,	false);
	...
	// fire start
	action				= new CObjectActionFire(missile,m_object,&m_storage,0,"throw start");
	add_condition		(action,id,eWorldPropertyHidden,	false);
	add_condition		(action,id,eWorldPropertyThrowStarted,false);
	...
	// fire idle
	action				= new CObjectActionThreaten(m_object,m_object,&m_storage,"throw idle");
	add_condition		(action,id,eWorldPropertyHidden,	false);
...
//	action				= new CSObjectActionBase(missile,m_object,&m_storage,"after_threaten");
